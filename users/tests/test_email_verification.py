...
class TestEmailVerification:
    ...
    def test_email_sent_on_send_confirmation(
        ...
        assert (
            len(mail.outbox) == 1
        ), "Verification email should be sent when send_confirmation is called"
    ...
    def test_verify_email_with_valid_key_returns_tokens(
        ...
        assert (
            "jwt-refresh-token" in verify_response.cookies
        ), "JWT refresh cookie should be set"
    ...
    def test_verify_email_with_invalid_key(self, api_client, verify_email_url):
    ...
    def test_verify_email_without_key(self, api_client, verify_email_url):
        """Test that verifying email without key returns error"""
        verify_response = api_client.post(
            verify_email_url, data=json.dumps({}), content_type="application/json"
        )
    ...
    def test_user_can_access_protected_endpoint_after_verification(
        ...
        user_response = api_client.get(
            user_details_url, HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
